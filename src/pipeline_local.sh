...
if [[ $HIST_FLAG -eq 1 ]]; then
    ...
    mrhistmatch nonlinear ./OUTPUTS/U.nii.gz ./OUTPUTS/D.nii.gz ./OUTPUTS/U_histmatched.nii.gz
    rm ./OUTPUTS/b0_all_masked.nii.gz
...
if [[ $TOPUP -eq 1 ]]; then
    ...
    if [[ $ODD_FLAG -eq 1 ]]; then
        ...
        --config=$Synb0_SRC/synb0_1.cnf --iout=./OUTPUTS/b0_all_topup.nii.gz --out=./OUTPUTS/topup $TOPUP_THREADS
    else
        topup -v --imain=./OUTPUTS/b0_all_masked.nii.gz --datain=./INPUTS/acqparams.txt \
